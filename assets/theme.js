...
(function(){
  ...
  theme.checkCartForCustomProducts = async function () {
    let cart = await theme.cart.getCart();
    let custom_product_exists = false;
    if (cart.items.length > 0) {
      for (let item of cart.items) {
        let product = await fetch('/products/' + item.handle + '.js');
        product = await product.json();
        if (product.tags.includes('hair-care')) {
          custom_product_exists = true;
          break;
        }
      }
    }

    if(!custom_product_exists){
      $('.quiz-button-show').attr('style', 'display: none !important');
      $('.cart__checkout-wrapper').attr('style', 'display: block !important');
      return;
    }

    if (!__st.cid) {
      $('.quiz-button-show').attr('style', 'display: block !important');
      $('.cart__checkout-wrapper').attr('style', 'display: none !important');
      return;
    }


    let quizSubmissions = await fetch('https://api-quiz.bareanatomy.in/submit/profiles?customer_id=' + __st.cid);
    quizSubmissions = await quizSubmissions.json()
    let customer_tag = quizSubmissions.data.customerData.tags;
    let quiz_id = 2;

    if (customer_tag.includes('gender-male')) {
      quiz_id = 3;
    }

    let quiz_found = 0;
    for (let item of quizSubmissions.data.submission) {
      if (item.quiz_id != quiz_id) {
        continue;
      }
      if (item.is_quiz_completed == 1) {
        quiz_found = 1;
        break;
      }
    }
  ...
  theme.sendAddToCartEvent = async function(){
    let cart           = await theme.cart.getCart();
    let tags           = [];
    ...
    for (let item of cart.items) {
      ...
      total_amount   += Number(item.price) / 100 * Number(item.quantity);
      titles.push(item.title);
      variant_ids.push(item.variant_id);
      product_ids.push(item.product_id);
      project_image.push(item.image);
      product_price.push((item.price) / 100);
      product_url.push('https://innovist.com' + item.url);
      try{
        let product = await fetch('/products/' + item.handle + '.js');
        product = await product.json();
    ...
    console.log(product_ids.join().split(","));
    ...
    fetch("https://api-automation.bareanatomy.in/api/webEngage/sendEvent", {
      ...
      body: JSON.stringify({
        ...
        eventData: {
            "Tags"           : tags.join(", "),
            "Line Item Names": titles.join().split(","),
            "Variant IDs"    : variant_ids.join(", "),
            "Product IDs"    : product_ids,
            "Product IDs new": product_ids.join().split(","),
            "Product Images" : project_image.join().split(","),
            "Product Name"   : titles.join().split(","),
            "Product Price"  : product_price.join().split(","),
            "Product URL"    : product_url.join().split(","),
  ...
  theme.pageTransitions = function() {
    if (document.body.dataset.transitions === 'true') {
      ...
      document.querySelectorAll('a:not(.js-no-transition, .jdgm-paginate__page, .jdgm-star)').forEach(el => {
  ...
  theme.sizeDrawer = function() {
    var toolbarHeight = document.getElementById('shopify-section-toolbar').offsetHeight;
    var header = document.getElementById('HeaderWrapper').offsetHeight + toolbarHeight;
...
$('.product-block.product-block--header.product-single__header.small--hide .jdgm-widget + a,.product-single__header--mobile.medium-up--hide .jdgm-widget + a').on('click', function(e) {
  e.preventDefault();
  $('.jdgm-widget .jdgm-prev-badge').trigger('click');
});
...
/* Festive gift code start */
 jQuery(document).ready(function($){

    $('.left_part .product_select .title_select').click(function(){
      $('.product_select').not($(this).parent()).removeClass('active');
      $(this).parent().toggleClass('active');
      $('.product_list').not($(this).siblings()).slideUp();
      $(this).siblings().slideToggle();
    });
    
    let all_iteam = [];
	
    function moveAlong(all_iteam_args) {
      //console.log(all_iteam_args);
	  if (all_iteam_args.length) {
	    var request = all_iteam_args.shift();
	    var data = 'id='+ request.varinat_id + '&quantity=1'
        var data = {
          "id": request.varinat_id,
          "quantity": 1,
          properties: { 'isProductBundle' : true }
        }
	    $.ajax({
	      type: 'POST',
          url: '/cart/add.js',
	      ...
	      success: function(res){          
	          moveAlong(all_iteam_args);
             if (all_iteam_args.length == 1){
              setTimeout(function(){
        ...
        error: function(){
          console.log('error', all_iteam_args) }
        });
      }
   }
   
   $('.show_builder').click(function() {
          $('.top_part').hide();
          $('.builder_part').show();
       window.scrollTo({top: 0, behavior: 'smooth'});
        });
   $('.hero_wrapper').click(function() {
          $('.top_part').hide();
          $('.builder_part').show();
       window.scrollTo({top: 0, behavior: 'smooth'});
        });
 ...
 $('.checkout').click(function(){
      $('body').addClass('cart-list-item');
      let all_selected = $(this).parents('.right_part').find('.selected-itam');
      var all_selected_title = [];
      var Shopify = Shopify || {};
      $.ajax({
        type: "POST",
        url: '/cart/clear.js',
        success: function(){
              $(all_selected).each(function(index) {
              var v_id = $(this).attr('data-v-id');
              all_iteam.push({
                  "id":v_id,
                  "quantity":1,
                   properties: { 'Festive offer' : true} 
              })
                //all_selected_title.push($(this).find('h3').text());
            });
          //var prod_01 = all_selected_title[0];
          //var prod_02 = all_selected_title[1];
          //var prod_03 = all_selected_title[2];
           // var main_data = {
             //   "id": $('.main_bundle_product_cart').data('id'),
              //  "quantity": 1,
               // properties: { 'Build Your Own Box' : true,'Product 1' : prod_01,'Product 2' : prod_02,'Product 3' : prod_03 } 
            //}
        ...
        dataType: 'json'
         ...
         $('.close-icon').click(function(){
           $(this).parent().removeClass('selected-itam');
           var product_attr_remove = $(this).attr('data-handle');
           $('[data-product='+product_attr_remove+']').find('.select_product').removeClass('selected');
           $('[data-product='+product_attr_remove+']').find('.select_product').text('Add to Cart');
           $('[data-product='+product_attr_remove+']').parent().find('.select_product').removeClass('disabled');
           $('.checkout').removeClass('active');
           var total_item_count = $('.selected-itam').length;
          if(total_item_count == 0){
            $('.right_part').removeClass('active');
          }
         });
    
        $('.btn.select_product').click(function(){           
          let p_handle = $(this).parents('li').attr('data-product');
          $(this).addClass('selected');
          $(this).text('Added to Cart');
          $('.right_part').find('.'+p_handle).addClass('selected-itam');
          let max_length_select = $(this).parents('.product_list').attr('data-product-limit');
          let select_length = $(this).parents('.product_list').find('.selected').length;
          let total_length = $(this).parents('.left_part').find('.selected').length;
          
          var total_item_count = $('.selected-itam').length;
          if(total_item_count >= 1){
            $('.right_part').addClass('active');
          }
          
          if(max_length_select >= select_length){
            let step = parseInt($(this).parents('.product_list').attr('data-stap'));
            let step_done = step - 1;
            if(max_length_select == select_length){
                $('.range-design li').eq(step_done).addClass('active');
                $('.product_select').eq(step).find('.title_select').trigger('click');
              }
          } else{
             $(this).removeClass('selected');
             $(this).text('Add to Cart');
             $('.right_part').find('.'+p_handle).removeClass('selected-itam');
             $(this).parents('.product-list-main').find('.btn').not($(this).parents('.product-list-main').find('.btn').hasClass('selected')).addClass('disabled');
          }
          setTimeout(function(){
          var total_s_l = $('.select_product.selected').length;
          if(total_s_l == 3 ){
            $('.checkout').addClass('active');
            $('.right_part a.btn.checkout').addClass('active');
            $('html,body').animate({
              scrollTop: ($(".right_part").offset().top - 150) },
            'slow');
          }else{
             $('.checkout').removeClass('active'); 
            $('.right_part a.btn.checkout').removeClass('active');
          }
            }, 100)
        });
    });
...
$(window).on('scroll', function(){
  var head_h = $('#shopify-section-header').innerHeight();
  var top_h = $(".stap_wrapper").offset().top;
  if($(this).scrollTop() >= top_h){
    $('.right_part').addClass('active-fixed');
    $('.right_part').css({"top": head_h})
  }else{
    $('.right_part').removeClass('active-fixed');
     $('.right_part').css({"top": "0"})
  }
})

$(window).on('load', function(){
  setTimeout(function(){
  $('#shopify-section-template--14483341836334__1641793755bc8dbb16').addClass('active');
  }, 200)
})

/* Festive gift code end */


$('.countdown-part').each(function(index){
  $(this).find('.days').attr('class','days'+index)
  $(this).find('.hours').attr('class','hours'+index)
  $(this).find('.minutes').attr('class','minutes'+index)
  $(this).find('.seconds').attr('class','seconds'+index)
  $(this).find('#countdown').attr('id','countdown'+index)
  $(this).attr('id','countdown-part'+index)
...
setTimeout(function(){
$('.countdown-part').each(function(index){
          var data_main_date = $(this).attr('data-date');
          var data_id = $(this).attr('id');
          console.log('innerText',document.getElementsByClassName("days"+index)[0].innerText);

          data_fun(data_main_date,data_id,index);
          
        })

        function data_fun(data_main_date,data_id,index){
  const second = 1000,
        minute = second * 60,
        hour = minute * 60,
        day = hour * 24;
  ...
  let today = new Date(),
      ...
      nextYear = yyyy + 1,
      dayMonth = "09/30/",
    // add date like this "03/30/2026"
      birthday = data_main_date;
  ...
  const countDown = new Date(birthday).getTime(),
      x = setInterval(function() {    
        ...
        document.getElementsByClassName("days"+index)[0].innerText = Math.floor(distance / (day)),
          document.getElementsByClassName("hours"+index)[0].innerText = Math.floor((distance % (day)) / (hour)),
          document.getElementsByClassName("minutes"+index)[0].innerText = Math.floor((distance % (hour)) / (minute)),
          document.getElementsByClassName("seconds"+index)[0].innerText = Math.floor((distance % (minute)) / second);
